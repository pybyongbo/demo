...
var J_KCWidget = (J_KCWidget||{}).prototype={
		...
		var fn = {
			load:function(t){
				...
				if (s.tri!=t&&t){
					...
					o.stop().show();
					if (s.fixed){
						J_KCWidget.J_Compatible(o.css({'position':'fixed','top':(parseInt(o.css('top'))-$(document).scrollTop())+'px'}),{'fixed':true});//,
					}
			...
			reset_position:function(){
				...
				var xy = (node==window)?{top:0,left:0}:tt.offset();
				...
				o.css({'position':'absolute','z-index':'10000','top':(top+u[1])+'px','left':(left+u[0])+'px',
						'clear':'both'});
			...
			mask_load:function(){
				$("<div class='"+this.maskCls+" J_KCWidget' data-widget-type='Compatible' data-widget-config=\"{fixed:true}\" style=\"position:absolute;left:0;top:0;z-index:9999;background:#000;\"></div>")
	...
	J_Accordion:function(o,$config){
		...
		check(o.children(s.triggerCls).eq(s.activeIndex-1));
	},
	...
	J_Carousel:function(o,$config){
		...
		if (s.effect=="fade"||s.effect=="none") c_l.hide();//.css({position: "absolute",top:"0",left:"0"})
			...
			if (s.effect=="scrollx"){c.css({width:"999999px"});
